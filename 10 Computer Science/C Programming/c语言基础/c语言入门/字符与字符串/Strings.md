...
指针用 `sizeof`，得到的只是**指针变量本身的大小**，而不是它指向的字符串长度。[[Unary Operators#^948eb7|sizeof计算变量类型的大小]]
...
**常见错误：** 在函数传参时，数组会退化为指针。[[Arrays#^29dcc1|数组的退化]]
...
- **`printf` (stdio.h):** 格式化输出到字符串。
  ...
  - *代码：* `sprintf(buffer, "Score: %d", 100);` -> buffer 内容变为 "Score: 100"。[[sprintf Function Examples]]
