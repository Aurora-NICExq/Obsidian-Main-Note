...
- 数组在作为参数传递给函数时，不会拷贝整个数组，而是**退化（decay）** 为指向数组第一个元素的指针。 ^29dcc1
- 因此，在函数内部**无法知道数组的真实长度**。使用`sizeof`操作符会得到指针的长度[[Array Exercises#^815a46|习题]]]
- **数组名是一个“不可修改的左值”（Non-modifiable L-value），因此它不能**像指针变量那样进行自增（`++`）、自减（`--`）或赋值操作。
### 数组名是什么？
```c
int main(){
	printf("%p",arr);
...
```
**结论：数组名是首元素的地址**[[Pointers#^062dc4|数组和指针的关系]]
```c
int n=sizeof(arr);
...
2. &数组名，此时也表示整个数组，取出整个数组的地址[[Array Exercises#^4180fb|习题1]]
```c
printf("%p",&arr);
